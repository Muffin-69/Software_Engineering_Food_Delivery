...
/* ──────────────────────────────────────────────────────────────
   Check Status — shows the customer's ONGOING orders
   (pending / preparing / on-the-way) with a Cancel button on
   the ones that haven't been dispatched yet.
   ────────────────────────────────────────────────────────────── */

const ACTIVE_STATUSES = new Set(["pending", "preparing", "on-the-way"]);
// Customers can only cancel orders that haven't started moving.
...
export default function CheckStatusPage({
  ...
  const ongoing = orders.filter((o) => ACTIVE_STATUSES.has(o.status));
      ...
      <div className="dashboard-main">
        ...
        <section className="owner-section">
          ...
          {loading && orders.length === 0 ? (
            ...
            <p className="owner-hint" style={{ paddingTop: 12 }}>
              No ongoing orders. Place one from the New Order screen!
            </p>
          ) : (
